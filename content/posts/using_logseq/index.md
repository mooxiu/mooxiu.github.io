---
title: "notion很好，所以我选择logseq"
date: 2021-12-16T22:17:13+08:00
tags: 
    - logseq
    - notion
    - 关于笔记的笔记
...
比如 [AppFlowy](https://github.com/AppFlowy-IO/appflowy)：
![AppFlowy](appflowy.png)

再比如微软的[Microsoft Loop](https://www.microsoft.com/en-us/microsoft-loop?ms.url=microsoftcommicrosoft-loop):
![Microsoft Loop](loop.png)
...
- notion 没有标准的markdown编辑器。我非常习惯在笔记中嵌入代码，插入引用，加入数学公式，插入超链接……这些在一个标准的markdown 编辑环境非常容易实现而且可以直接在键盘上完成，根本不需要对着菜单的选项点来点去。**点击右键，选择插入超链接，粘贴地址，再加上标题**这样的操作简直是Microsoft Word的编辑体验。也许它有自己的一套快捷键，但是既然业界已经有标准，为什么还要恶心你？
...
- 它支持标准的markdown，同时你也可以单纯将它作为一个索引工具，你可以在特定的文件夹下面使用任何软件编辑你的`.md`文件，比如`typora` 或者 `VSCode`。
- 如果你想要同步你的文件，你可以使用自己的网盘或者干脆把github当作你的网盘，它在后台会自动使用git来版本控制你的变更。
...
- 它是一个[开源的项目](https://github.com/logseq/logseq)，而且用Clojure写的，非常酷。
...
比如我最近在学习编译原理，每天临时看到东西会打一个`compilers`的tag，比如这是14号的笔记：
![14日的笔记](note_14.png)
...
![关于这个主题的所有记录](compilers.png)

## 不足

这个产品还在开放中，截至2021年12月16日，这个软件的最新版本号是`0.5.4`，还是早期的阶段。我之前有看到说这个软件会丢笔记，但我也看到这个软件开发者迅速回应并在当天修复了这个问题。目前来说，这个软件存在的不足之处有：
